...
function setup() {
    ...
    for (let x = unit*2; x <= windowWidth - 400; x += unit) {
        for (let y = unit*2; y <= windowHeight - 400; y += unit) {
            herbages.push(new herbage(x, y, 1));
        }
    }
    
    herbages.push(new herbage (200,200, 1));
    herbages.push(new herbage (400,600, 1));
    herbages.push(new herbage (800,100, 1));
...
function draw() {
    ...
    for (let i = 0; i < herbages.length; i++) {
        
        if (herbages[i].regrows <= 0) {
            herbages.splice(i,1);
        } else {
            herbages[i].update();
            herbages[i].show();
...
class prey extends animal {
    ...
    update() {
        ...
        if (this.hunger >= 100) {
            herbages.push(new herbage(this.pos.x, this.pos.y, 0));
            this.hunger -= 30;
        }
...
class herbage {
    constructor (x, y, growth) { //xy is position, regrow is number of frames required to regrow
        this.pos = createVector(x,y);
        this.regrowTime = 500; //frames required to regrow
        this.growth = growth; //shows state of growth
        this.regrows = 2;
    }
